    ...
    return {
      ...
      link: function (scope, element) {
        ...
        function render_panel() {
          ...
          var nodes = svg.append("g").datum(scope.data).selectAll("path")
            .data(partition.nodes);

          var labels = svg.append("g").datum(scope.data).selectAll("path")
            .data(partition.nodes);
          ...
          if (scope.panel.labels) {
            labels.enter()
              .append("g")
              .each(function (node) {
                if (node.depth > 0 && node.depth <= scope.panel.label_rings) {
                  d3.select(this).append("text")
                    .style("font", "11px sans-serif")
                    .style("font-weight", scope.panel.bold_labels ? "bold" : "normal")
