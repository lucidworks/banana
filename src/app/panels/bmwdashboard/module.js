        ...
        if ($scope.panel.mode === 'value' || $scope.panel.mode === 'counts') {
            ...
            _.each(arr_id, function (id) {
                if (id === 0) {
                    ...
                    var temp_q = 'q=' + $scope.panel.value_field3 + '%3A%5B' + '*' + '%20TO%20' + '*' + '%5D' + wt_json + rows_limit + fq + facet + '&fl=' + time_field + ' ' + $scope.panel.value_field3;
      ...
      link: function(scope, elem) {
		...
		APdex_1 = parseInt(100*(AP_1+AP_2)/AP_n);
		 ...
		 var AP_1_conn = 0.0;
		  ...
		  for (var i = 0; i < chartData[1].length; i++) {
			  if(parseInt(chartData[1][i].connectElapsed)<=scope.panel.threshold_first ){
			  AP_1_conn+=1;
			  }else if(parseInt(chartData[1][i].connectElapsed)<scope.panel.threshold_second && parseInt(chartData[1][i].connectElapsed)>scope.panel.threshold_first){
			  AP_2_conn+=1*0.5;
			  }
		  }
		var APdex_conn =100;
		...
		var AP_1_cpu = 0.0;
		  ...
		  for (var i = 0; i < chartData[2].length; i++) {
			  if(parseInt(chartData[2][i].cpu)<=scope.panel.cpu_first){
			  AP_1_cpu+=1;
			  }else if(parseInt(chartData[2][i].cpu)<scope.panel.cpu_second&& parseInt(chartData[2][i].cpu)>scope.panel.cpu_first){
			  AP_2_cpu+=1*0.5;
			  }
		  }
		var APdex_cpu =100;
		...
		var AP_1_me = 0.0;
		  ...
		  for (var i = 0; i < chartData[3].length; i++) {
			  if(parseInt(chartData[3][i].UsedMemery)<=scope.panel.memory_first ){
			  AP_1_me+=1;
			  }else if(parseInt(chartData[3][i].UsedMemery)<scope.panel.memory_second && parseInt(chartData[3][i].UsedMemery)>scope.panel.memory_first){
			  AP_2_me+=1*0.5;
			  }
		  }
		var APdex_me =100;
