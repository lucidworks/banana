...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  module.controller('multihistogram', function($scope, $q, $timeout, timer, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
    ...
    $scope.get_data = function(segment, query_id) {
        ...
        metrics = $scope.panel.value_field.split(","); //字符分割
        var metricColors = $scope.panel.chartColors;
        //var metrics = ['SQLConnections','SQLQueries','SQLTotalConnectionOpenTime','ActionTime','RequestTime'];
        var mypromises = [];
        _.each(metrics, function (metirc) {
            var temp_q = 'q=*%3A*' + wt_json + rows_limit + fq + facet + '&fl=rs_timestamp ' + metirc;
        ...
        if (dashboard.current.services.query.ids.length >= 1) {
            $q.all(mypromises).then(function (results) {
...

                {
                    ...
                    if (!(_.isUndefined(results[0].error))) {
                        $scope.panel.error = $scope.parse_error(results[0].error.msg);
                        return;
                    }
                    ...
                    if ($scope.panel.mode === 'values') {
                        // Group By Field is not specified//#7EB26D, #EAB839, #6ED0E0, #EF843C, #E24D42
                        //var metrics = ['SQLConnections','SQLQueries','SQLTotalConnectionOpenTime','ActionTime','RequestTime'];
                        var arry = new Array();
                        var col = 0;
                        _.each(metrics, function (attr) {
                            var object = new Object();
                            object.alias = attr;
                            object.color = metricColors[col];
                            col++;
                            arry.push(object);
                        });
                        for (var b = 0; b < metrics.length; b++) {
                            entries = results[b].response.docs;
                            ...
                            for (var j = 0; j < entries.length; j++) { // jshint ignore: line
                                ...
                                entry_value = entries[j][metrics[b]];
  ...
  module.directive('multihistogramChart', function(dashboard, filterSrv) {
    return {
      restrict: 'A',
      template: '<div></div>',
      link: function(scope, elem) {
        ...
        function render_panel() {
          ...
          // Set barwidth based on specified intervalss
