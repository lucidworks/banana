	  ...
	  if ($scope.panel.mode === 'value' || $scope.panel.mode === 'counts'){
		  ...
		  if($scope.panel.reverse === 1){
		   ...
		   _.each(arr_id, function(id) {
				...
				var temp_q = 'q='+$scope.panel.value_field + '%3A%5B' +threshold_1+'%20TO%20'+ threshold_2 +'%5D'+wt_json + rows_limit + fq + facet + values_mode_query;
				}else if(id ===2){
				...
				staticfield = $scope.panel.value_field1;
				}
          ...
          // if ($scope.query_id === query_id && _.difference(facetIds, $scope.panel.queries.ids).length === 0) {
            ...
            _.each($scope.panel.queries.ids, function(id,index) {
			  ...
			  if($scope.panel.segment == 4 ){
				   var entry_time1, entries1, entry_value1; 
						var hits1=0;
					  var arry = new Array();
					...
					bmw_total = entries1[0][$scope.panel.value_field1][0];
        ...
        function render_panel() {
          ...
          // Populate from the query service
		   ...
		   _.each(scope.data, function(seri) {
					if (seri.info.alias != scope.panel.value_field1){
					num_all += seri.hits;}
					});
          try {
            _.each(scope.data, function(series) {
				series.color = series.info.color;
				if (series.info.alias == scope.panel.value_field1){
					if(scope.panel.value_field === 'UsedMemery'){
						series.info.alias = "TotalMB";
					}else if(scope.panel.value_field === 'FreeDiskSpace'){
						series.info.alias = "TotalGB";
					}else{
			  ...
			  series.hits = mid;
			  if(series.info.color == "#E5AC0E"){
				  series.info.alias = "Warning% ";
			  }else if(series.info.color == "#1F78C1"){
				  series.info.alias = "Normal% ";
			  }else if(series.info.color == "#EA6460"){
		   ...
		   if(scope.panel.mode === 'value'){
			   ...
			   xLabel = "Threshold:"+' '+scope.panel.threshold_first+' and '+scope.panel.threshold_second+' and '+scope.panel.threshold_third; 
				...
				if(scope.panel.reverse){
					xLabel = "Warning:>"+' '+scope.panel.threshold_second+xunit+' and <='+scope.panel.threshold_first+xunit+';'+'Risk:<='+scope.panel.threshold_second+xunit; 
				   }
